...
exports.validate = function (options) {
  return new Promise(function (resolve, reject) {
    ...
    options.timestamp = moment().format('LLL');
...
exports.writeReport = function (options) {
  var mainTpl = Template.load(options.template || Template.defaultTemplate);
  var navigatorTpl = Template.load(Template.baseTemplate('navigator.hbs'));
  var contentTpl = Template.load(Template.baseTemplate('content.hbs'));

  handlebars.registerPartial('navigator', navigatorTpl);
  handlebars.registerPartial('content', contentTpl);

  handlebars.registerHelper('encodeHTML', function (inputData) {
    return new handlebars.SafeString(inputData);
  });

  handlebars.registerHelper('writeTag', function (inputData) {
    return inputData.replace('@', '');
  });

  handlebars.registerHelper('stripStacktrace', function (inputData) {
    /*
    const lines = inputData.split(/\r?\n/);
    let message = '';
    for (var i = 0; i < lines.length; i++){
      if (lines[i] && lines[i].trim().startsWith('at ')) {
        continue;
      }
      message += lines[i] + '\r\n';
    }
  ...
  var template = handlebars.compile(mainTpl, { strict: false });
  var html = template(options);
  return writeHTML(options.dest, options.name, html);
};
...
function stepDurationConverter(step, isCucumber2) {
  ...
  if (duration) {
    step.result.durationFormatted = prettyMs(duration / 1000000000);
  }
  /*
  if (Duration.isMinuteOrMore(duration, isCucumber2)) {
    // If the test ran for more than a minute, also display minutes.
...
function writeHTML(targetDirectory, reportName, html) {
  return new Promise(function (resolve, reject) {
    var fileName = path.join(targetDirectory, reportName || 'index.html');
...
function handleEmbeddingPlainText(embedding, element) {
  var parsedData = JSON.parse(embedding.data);

  // Save plain text on element so we use it in HTML
  element.plainTextEmbeddings = element.plainTextEmbeddings || {};

  //var decodedText = atob(embedding.data);
  element.plainTextEmbeddings[parsedData.id] = parsedData.text;
}
